...
const runSeed = async () => {
  ...
  const optionVotes = [
    otherUsers.slice(0, 4).map((usr: any) => usr.id), // first option
    otherUsers.slice(4, 6).map((usr: any) => usr.id), // second option
    otherUsers.slice(6, 10).map((usr: any) => usr.id), // third option
    [], // fourth option
  ] as const;
